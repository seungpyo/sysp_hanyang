...
void eval(char *cmdline) 
{
    ...
    if(pid < 0) { // Failed to fork.
       ...
       if(!bg) {
           ...
           if(cur_job != NULL && cur_job->state != ST) {
               kill(pid, SIGKILL);
               deletejob(jobs, pid);
           }
       } else {
...
void do_bgfg(char **argv) 
{
    ...
    /* bg command */
    if (!strcmp(argv[0], "bg")) { 
	if (kill((jobp->pid), SIGCONT) < 0) // Why negative?
...
void sigint_handler(int sig) 
{
    ...
    pid = fgpid(jobs);
    //listjobs(jobs);
    //printf("sigint handler found %d as a fgpid.\n", pid);

    if (verbose)
	printf("sigint_handler: entering\n");
    if (pid > 0) {
	if (kill(pid, SIGINT) < 0) // Why (negative) -pid?
...
void sigtstp_handler(int sig) 
{
    ...
    if ((pid = fgpid(jobs)) > 0) {
	if (kill(pid, SIGTSTP) < 0) // Why negative?
